...
public class GestorHuespedes implements IGestorHuespedes {
  ...
  private void verificacionesDeCampos(HuespedDTO dto) {
    ...
    if (dto.getCategoriaFiscal() == CategoriaFiscal.RESPONSABLE_INSCRIPTO
      ...
      throw new CuitVacioException(
          "Error, el CUIT no puede estar vacio si el huesped es" + "responsable inscripto");
    }
  }
  ...
  public List<HuespedDTO> buscar_huespedes(BusquedaHuespedDTO busqueda) {
    ...
    boolean hayCriterio = notBlank(datos.getApellido()) || notBlank(datos.getNombres()) ||
    ...
    return huespedDAO.findAll(Example.of(huespedProbe, matcher)).stream().map(huespedMapper::toDTO)
        .toList();
  ...
  public HuespedDTO buscarPorId(IdentificacionHuespedDTO id) {
    Huesped huesped = huespedDAO
        .findByTipoDocumentoAndNumeroDocumento(id.getTipoDocumento(), id.getNumeroDocumento())
        .orElse(null);
    if (huesped == null) {
      return null;
    }
  ...
  public Huesped obtenerPorId(IdentificacionHuespedDTO id) {
    Huesped huesped = huespedDAO
        .findByTipoDocumentoAndNumeroDocumento(id.getTipoDocumento(), id.getNumeroDocumento())
        .orElseThrow(
            () -> new IllegalArgumentException("No se encontró huésped con ID proporcionado"));
    return huesped;
  }

  public Map<HuespedID, Huesped> obtenerMapaPorIds(List<IdentificacionHuespedDTO> idsHuesped) {
    Map<HuespedID, Huesped> mapa = new java.util.HashMap<>();
    for (IdentificacionHuespedDTO idDTO : idsHuesped) {
      HuespedID id = huespedMapper.toId(idDTO);
      Huesped huesped = obtenerPorId(idDTO);
      mapa.put(id, huesped);
    }
    return mapa;
  }
